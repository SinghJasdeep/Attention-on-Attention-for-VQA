...
class Model(nn.Module):
    ...
    def forward(self, v, b, q, labels):
        ...
        att = self.v_att(v, q_emb) # [batch, 36, 1]
...
class Model_2(nn.Module):
    ...
    def forward(self, v, b, q, labels):
        ...
        att_1 = self.v_att_1(v, q_emb) # [batch, 36, 1]
        att_2 = self.v_att_2(v, q_emb)  # [batch, 36, 1]

        att_1_copy = att_1.data.cpu().numpy()[:]
        att_2_copy = att_2.data.cpu().numpy()[:]

        for i in range(att_2_copy.shape[0]):
            plot_heatmap(att_1_copy[i], att_2_copy[i], title=str(i), saveLoc='HeatMaps/'+str(i))
